...
std::vector<std::vector<bool>> RandomGreedy(Input& input){
    int nCaches=input.cacheServer,nVideos=input.videos,nRequests=input.requests;
    std::vector<std::vector<bool>> bits(nCaches+1,std::vector<bool>(nVideos,false));
    auto vecRequests=input.requestsDescription;
    std::sort(vecRequests.begin(),vecRequests.end(), [](const std::vector<int>& v1, const std::vector<int>& v2) {
        return v1[2]<v2[2];
    });
    ...
    for(auto req: vecRequests){
        ...
        if(tempVec.empty())continue;
        int toSelect=min(5,(int)tempVec.size());
        int selectedInd=tempVec[rand(0,toSelect-1)].second;
        if(bits[selectedInd][video])continue;
